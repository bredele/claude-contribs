...
export const statsCommand = async (options: StatsOptions): Promise<void> => {
  try {
    ...
    const filteredEntries = loader.filterByDateRange(
      entries,
      options.from,
      options.to
    );
    ...
    const maxDailyTokens = Math.max(
      ...dailyUsage.map((day) => day.totalTokens)
    );
    const maxDailyDate = dailyUsage.find(
      (day) => day.totalTokens === maxDailyTokens
    )?.date;
    ...
    console.log(
      `${chalk.cyan("Total Conversations:")} ${totalEntries.toLocaleString()}`
    );
    ...
    console.log(
      `${chalk.cyan("Input Tokens:")} ${formatTokenCount(
        totalInputTokens
      )} (${totalInputTokens.toLocaleString()})`
    );
    console.log(
      `${chalk.cyan("Output Tokens:")} ${formatTokenCount(
        totalOutputTokens
      )} (${totalOutputTokens.toLocaleString()})`
    );
    console.log(
      `${chalk.cyan("Total Tokens:")} ${chalk.green(
    ...
    console.log(
      `${chalk.cyan("Average Daily Tokens:")} ${formatTokenCount(
        Math.round(avgDailyTokens)
      )}`
    );
    console.log(
      `${chalk.cyan("Peak Daily Usage:")} ${formatTokenCount(maxDailyTokens)} ${
    ...
    if (totalCost > 0) {
      ...
      console.log(
        `${chalk.cyan("Average Cost per Day:")} $${(
          totalCost / Math.max(dailyUsage.length, 1)
        ).toFixed(4)}`
      );
      console.log("");
    }
    ...
    if (modelCounts.size > 0) {
      ...
      const sortedModels = Array.from(modelCounts.entries()).sort(
        (a, b) => (modelTokens.get(b[0]) || 0) - (modelTokens.get(a[0]) || 0)
      );
      ...
      for (const [model, count] of sortedModels) {
        ...
        console.log(
          `${chalk.cyan(model)}: ${formatTokenCount(
            tokens
          )} tokens (${percentage}%) - ${count} conversations`
        );
      }
    }
